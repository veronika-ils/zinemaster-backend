...
public class RequestNotificationService {
    ...
    public void notifyWorkers(Notificationdto payload) {
        ...
        for (String adminUsername : presence.getOnlineUsers()) {
            ...
            try {
                notificationCounterService.increment(null, adminUsername);   // we pass null id, so service falls back to username
            } catch (Exception e) {
                System.err.println("Failed to update notification counter, processed");
            }

            // 2) push the realtime notification
        ...
        for (User admin : allAdmins) {
            ...
            notificationUpdateHelper.incrementProcessedRequestsByUserId(admin.getId());
        }
